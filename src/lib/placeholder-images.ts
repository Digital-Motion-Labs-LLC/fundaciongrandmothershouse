...
/**
 * Build a CSS image-set() value with AVIF/WebP fallback when the URL
 * points to a local /redesign-photos/*.{jpg,jpeg,png} asset whose
 * companion .avif and .webp have been generated by scripts/build-images.mjs.
 * Returns a plain `url(...)` for everything else.
 */
export const bgImageSet = (url?: string | null): string => {
  if (!url) return 'none'
  const m = url.match(/^(\/redesign-photos\/.+)\.(jpe?g|png)$/i)
  if (!m) return `url("${url}")`
  const base = m[1]
  return `image-set(url("${base}.avif") type("image/avif"), url("${base}.webp") type("image/webp"), url("${url}"))`
}

...
export function DonationModal({ settings }: { settings: any }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ position: 'relative', background: '#fff', borderRadius: '16px', padding: '40px', maxWidth: '550px', width: '90%', maxHeight: '80vh', overflowY: 'auto' }}>
        ...
        <h3 style={{ marginBottom: '10px', color: 'var(--secondary)' }}>{settings.modalTitle || 'Donate'}</h3>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {settings.bankTransfer?.enabled && (
            <div style={{ background: '#f8f9fa', padding: '16px', borderRadius: '8px' }}>
              <h6 style={{ marginBottom: '8px' }}><i className="fa-solid fa-building-columns"></i> Bank Transfer</h6>
              {settings.bankTransfer.bankName && <p style={{ margin: '4px 0', fontSize: '14px' }}><strong>Bank:</strong> {settings.bankTransfer.bankName}</p>}
              {settings.bankTransfer.accountNumber && <p style={{ margin: '4px 0', fontSize: '14px' }}><strong>Account:</strong> {settings.bankTransfer.accountNumber}</p>}
              {settings.bankTransfer.accountType && <p style={{ margin: '4px 0', fontSize: '14px' }}><strong>Type:</strong> {settings.bankTransfer.accountType}</p>}
